...
_CONFIRMATIONS = {
    "resolve-bases": "RESOLVE BASE DIGESTS",
    "build-images": "PUBLISH BUILDER IMAGES",
}
...
def bootstrap_matrix(*, lock: Path, stage: str, confirmation: str) -> str:
    """Validate a bootstrap stage and return its exact hosted-runner matrix."""

    if _CONFIRMATIONS.get(stage) != confirmation:
        raise ValidationError("builder bootstrap confirmation is not exact")
...
def _parser() -> argparse.ArgumentParser:
    ...
    matrix.add_argument("--stage", choices=tuple(_CONFIRMATIONS), required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if args.command == "matrix":
            print(
                bootstrap_matrix(
                    lock=args.lock, stage=args.stage, confirmation=args.confirmation
                )
            )
