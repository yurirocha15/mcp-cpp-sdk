...
int main() {
    ...
    server.add_tool<AddInput, AddOutput>(
        "add", "Add two numbers", std::move(add_schema),
    ...
    server.add_tool<GreetInput, GreetOutput>(
        "greet", "Greet someone by name", std::move(greet_schema),
        [](Context& /*ctx*/, GreetInput in) -> GreetOutput {
    ...
    server.add_tool<nlohmann::json, nlohmann::json>("echo_async", "Echo the input asynchronously",
                                                    std::move(echo_schema),
    ...
    server.add_tool<nlohmann::json, nlohmann::json>(
        "log_and_upper", "Uppercase the input and log via context", std::move(upper_schema),
    ...
    server.add_resource<ReadResourceRequestParams, ReadResourceResult>(
        std::move(config_resource), [](ReadResourceRequestParams params) -> ReadResourceResult {
    ...
    server.add_resource_template(std::move(user_template));
    ...
    server.add_prompt<GetPromptRequestParams, GetPromptResult>(
        std::move(review_prompt), [](GetPromptRequestParams params) -> GetPromptResult {
