#!/usr/bin/env python3
"""Reject tracked private keys and GitHub or ConanCenter credentials."""
...
SENSITIVE_PATTERN = re.compile(
    b"|".join(
        (
            ...
            rb"CONAN_CENTER_PR_BOT_PA" rb"T=",
        )
    )
)
...
def main() -> int:
    ...
    if matches:
        for path in matches:
            print(
                f"sensitive material pattern found in tracked file: {str(path)!r}",
                file=sys.stderr,
            )
        return 1
    return 0
