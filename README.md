...
Package-manager routes are documented in [the installation guide](docs/installation.rst). The GitHub, Cloudsmith (APT/RPM), and AUR jobs read the exact published object back before recording `PUBLISHED`. ConanCenter, Homebrew, and Chocolatey remain pending while their provider review or moderation is outstanding. Install only after the requested version appears at the destination; an upload, pending ledger entry, or open registry PR is not availability proof.
